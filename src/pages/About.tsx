...
import { 
  ...
  Lightbulb, Rocket, Activity, Target, Layers, BarChart3, Database, Fingerprint
} from 'lucide-react';
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full lg:w-1/2 relative">
               <div className="aspect-square w-full max-w-md mx-auto relative group">
                  ...
                  {/* Holographic Infrastructure Visual */}
                  <div className="absolute inset-4 rounded-full border-[1px] border-ai-cyan/30 flex items-center justify-center relative">
                     <div className="absolute inset-8 rounded-full border-[1px] border-dashed border-electric-blue/40 animate-[spin_40s_linear_infinite]" />
                     <div className="absolute inset-16 rounded-full border-[1px] border-violet-glow/30 animate-[spin_30s_linear_infinite_reverse]" />
                     <div className="w-32 h-32 rounded-full bg-midnight-black border border-glass-border flex items-center justify-center shadow-[0_0_50px_rgba(37,99,235,0.3)] z-10">
                        <Cpu className="w-12 h-12 text-ai-cyan animate-pulse" />
                     </div>
                     
                     {/* Orbiting nodes */}
                     {[0, 1, 2, 3].map((i) => (
                       <div key={i} className="absolute inset-0 animate-[spin_20s_linear_infinite]" style={{ animationDelay: `-${i * 5}s` }}>
                         <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 bg-midnight-black border border-ai-cyan rounded-full shadow-[0_0_15px_rgba(0,229,255,0.8)]" />
                       </div>
                     ))}
                  </div>
               </div>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full lg:w-1/3">
               ...
               <div className="space-y-4">
                 {[ 'AfriTrade OS Analytics', 'NeuroSmart Dashboard', 'IPOS Enterprise Core' ].map((item, i) => (
